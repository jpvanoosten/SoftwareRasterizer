...
int main( int argc, char* argv[] )
{
    ...
    CameraController camera { { 0, 0, 5 } };
    camera.setPerspective( 60.0f, static_cast<float>( WINDOW_WIDTH ) / WINDOW_HEIGHT, 1.0f, 100.0f );
    ...
    Model quad { "assets/models/cube.obj" };
    ...
    while ( window )
    {
        ...
        // Draw the cube.
        const glm::mat4 modelMatrix            = glm::rotate( glm::rotate( pitch, glm::vec3 { 1, 0, 0 } ), yaw, glm::vec3 { 0, 1, 0 } );
        vertexShader.modelViewProjectionMatrix = camera.getViewProjectionMatrix() * modelMatrix;
        vertexShader.instanceId                = 0;
        for ( const auto& mesh: quad.getMeshes() )
        {
        ...
        for ( int i = 0; i < WINDOW_WIDTH * WINDOW_HEIGHT; ++i )
        {
            if ( auto c = visibilityBuffer[i]; c.primitiveId < 0xffff )
            {
                ...
                {
                    colorBuffer[i] = randomColors[c.instanceId * 6 + c.primitiveId];  // mat->diffuseColor;
                }
            }
        }
